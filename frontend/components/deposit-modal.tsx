...
const ASSET_META: Record<
  string,
  { proto: string; apy: string; dot: "green" | "blue" | "amber"; note: string }
> = {
  LBTC: {
    proto: "Lombard Protocol",
    apy: "4.8%",
    dot: "green",
    note: "Staking yield",
  },
  ...
  "SOLVBTC.B": {
    proto: "Solv Protocol (BNB)",
    apy: "5.4%",
    dot: "blue",
    note: "Cross-chain",
  },
  SBTC: {
    proto: "Stacks Protocol",
    apy: "4.5%",
    dot: "green",
    note: "L2 native",
  },
  SBTC_TEST: {
    proto: "BitflowOS Sepolia",
    apy: "5.47%",
    dot: "amber",
    note: "Test wrapper",
  },
  SBTC_EKUBO_TEST: {
    proto: "BitflowOS Ekubo Test",
    apy: "5.47%",
    dot: "blue",
    note: "Controlled pool",
  },
  BTCB: {
    proto: "Binance Wrapped BTC",
    apy: "4.4%",
    dot: "green",
    note: "CEX-backed",
  },
  BTC: {
    proto: "Native Bitcoin · Atomiq",
    apy: "5.47%",
    dot: "amber",
    note: "Atomic bridge",
  },
...
export function DepositModal({
  ...
  const [userVaultState, setUserVaultState] = useState<VaultState | undefined>(
    vaultState,
  );
  ...
  const route =
    source === "native" || normalizedAsset === "BTC"
      ? "BTC -> Atomiq -> SBTC -> Vault"
      : `${asset} -> StarkZap -> Vault`;
  const liveAsset = userVaultState?.assets.find(
    (item) => item.symbol.toUpperCase() === normalizedAsset,
  );
  ...
  const walletAddress =
    wallet?.address ??
    (typeof window === "undefined" ? "" : readWalletAddress());
    ...
    try {
      if (source !== "native" && normalizedAsset !== "BTC") {
        ...
        if (BigInt(amountBaseUnits) > BigInt(walletBalance || "0")) {
          throw new Error(
            `Deposit amount exceeds your ${asset} wallet balance.`,
          );
        }
  ...
  async function confirmAndSign() {
    ...
    try {
      if (source === "native" || normalizedAsset === "BTC") {
        if (!walletAddress) {
          setResult(
            "Connect a Starknet wallet before creating the Atomiq quote.",
          );
          return;
        }
        ...
        setResult(
          `Atomiq quote ${intent.id}: send BTC to ${intent.paymentAddress ?? "quoted address"}.`,
        );
        return;
      }

      const normalizedAmount = normalizeAmountInput(
        amount || "0",
        tokenDecimals,
      );
      const amountBaseUnits = decimalToBaseUnits(
      ...
      console.log("deposit calls:", JSON.stringify(calls.calls, null, 2));
      ...
      setResult(
        `Deposit submitted. StarkZap multicall ${shortHash(execution.hash)} is being watched by the agent.`,
      );
      if (walletAddress) {
      ...
      window.dispatchEvent(
        new CustomEvent("bitflowos:deposit-submitted", { detail }),
      );
      window.dispatchEvent(
        new CustomEvent("bitflowos:vault-refresh", { detail }),
      );
      window.setTimeout(() => {
        closeModal();
      }, 1100);
    ...
    open && typeof document !== "undefined"
      ? createPortal(
          <div
            ...
            onClick={(event) =>
              event.target === event.currentTarget ? closeModal() : undefined
            }
          >
            <div className="deposit-modal">
              <div className="m-head">
                <div className="m-head-l">
                  ...
                  <div>
                    ...
                    <div className="m-step-label">
                      {step === 1
                        ? "Step 1 of 2 - Choose asset & amount"
                        : step === 2
                          ? "Step 2 of 2 - Review before signing"
                          : "Signing transaction"}
                    </div>
                  </div>
                </div>
              ...
              <div className="m-steps">
                <div
                  className={`msd ${step > 1 ? "done" : step === 1 ? "active" : ""}`}
                />
                <div
                  className={`msd ${step > 2 ? "done" : step === 2 ? "active" : ""}`}
                />
              ...
              {step === 1 ? (
                <div className="m-body">
                  <div className="m-src">
                    <button
                      className={`m-src-btn ${source === "wrap" ? "act" : ""}`}
                      onClick={() => setModalSource("wrap")}
                      type="button"
                    >
                      <Send size={12} />
                      Wrapped BTC
                    ...
                    <button
                      className={`m-src-btn ${source === "native" ? "act" : ""}`}
                      onClick={() => setModalSource("native")}
                      type="button"
                    >
                      <Bitcoin size={12} />
                      Native BTC - Atomiq
                  ...
                  <div
                    className={`m-native-note ${source === "native" ? "show" : ""}`}
                  >
                    <strong>Native Bitcoin via Atomiq bridge.</strong> Send BTC
                    from your Bitcoin wallet. Atomiq swaps it cross-chain
                    atomically - it arrives as SBTC and routes straight into the
                    vault. No wrapping steps for you.
                  </div>
                  ...
                  <div className="m-f">
                    ...
                    <div className="sel-wrap">
                      <button
                        className={`sel-btn ${dropdownOpen ? "open" : ""}`}
                      ...
                      >
                        <div className="sel-btn-left">
                          ...
                          <div>
                            ...
                            <div className="sel-proto">
                              {selected.proto} - {selected.apy} APR
                            </div>
                          </div>
                        </div>
                        ...
                        >
                          <div className="dd-item-l">
                            ...
                            <div>
                              ...
                              <div className="dd-proto">
                                Native Bitcoin - via Atomiq
                              </div>
                            </div>
                          </div>
                          <div className="dd-item-r">
                  ...
                  <div className="m-f">
                    ...
                    <div className="m-hint">
                      <span>
                        Balance: {displayWalletBalance} {asset}
                      </span>
                      <button
                        className="ml"
                        onClick={() => setAmount(displayWalletBalance)}
                        type="button"
                      >
                        USE MAX
                      </button>
                  ...
                  <div className="m-route-info">
                    <div className="mri-row">
                      <span className="mri-l">ROUTE</span>
                      <span className="mri-r">{route}</span>
                    </div>
                    <div className="mri-row">
                      <span className="mri-l">GAS</span>
                      <span className="mri-r green">Sponsored by AVNU</span>
                    </div>
                    <div className="mri-row">
                      <span className="mri-l">TIME</span>
                      <span className="mri-r">~15 seconds</span>
                    </div>
                  </div>
                ...
                <div className="m-confirm">
                  <div className="mc-sum">
                    <div className="mc-row">
                      <span className="mc-l">ASSET</span>
                      <span className="mc-r amber">{asset}</span>
                    </div>
                    <div className="mc-row">
                      <span className="mc-l">AMOUNT</span>
                      <span className="mc-r">
                        {formatAmount(amount)} {asset}
                      </span>
                    </div>
                    <div className="mc-row">
                      <span className="mc-l">YOU RECEIVE</span>
                      <span className="mc-r amber">{receiveAmount}</span>
                    </div>
                    <div className="mc-row">
                      <span className="mc-l">SHARE PRICE</span>
                      <span className="mc-r">1.0044 BTC</span>
                    </div>
                    <div className="mc-row">
                      <span className="mc-l">ROUTE</span>
                      <span className="mc-r">{route}</span>
                    </div>
                    <div className="mc-row">
                      <span className="mc-l">GAS</span>
                      <span className="mc-r green">Sponsored - $0.00</span>
                    </div>
                    <div className="mc-row">
                      <span className="mc-l">STRATEGY</span>
                      <span className="mc-r">AI-Managed - TEE Attested</span>
                    </div>
                  </div>
              ...
              {step === 3 ? (
                <div className="m-body signing-body">
                  ...
                  <div className="sign-meta">
                    GAS SPONSORED BY AVNU - NO ETH NEEDED
                  </div>
                  {result ? <div className="sign-result">{result}</div> : null}
                  {busy ? (
                    <div className="sign-result">
                      Preparing wallet action...
                    </div>
                  ) : null}
                </div>
              ) : null}
              ...
              <div className="m-foot">
                BitflowOS does not custody assets. The smart contract vault
                holds all funds on-chain.
              </div>
            </div>
          </div>,
          document.body,
...
function decimalToBaseUnits(value: string, decimals: number) {
  ...
  if (fraction.length > decimals) {
    throw new Error(
      `Amount supports up to ${decimals} decimals for this asset.`,
    );
  }
  const base = `${whole}${fraction.padEnd(decimals, "0")}`.replace(
    /^0+(?=\d)/,
    "",
  );
